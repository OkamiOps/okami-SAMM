...
async function json(path, opts) { const r = await fetch(base + path, opts); return { status: r.status, body: await r.json().catch(() => null) }; }
...
(async () => {
  ...
  try {
    ...
    const prev = await fetch(`${base}/api/report/preview.pdf`, POST('', { state }));
